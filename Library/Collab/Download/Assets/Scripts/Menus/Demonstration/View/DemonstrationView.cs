    ...
    public class DemonstrationView : DemonstrationElement
    {
        ...
        public void RefreshGestures()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                if (predictedGesture == i && i != 0)
                {
                    ...
                    if (app.model.GetPredictedGestures().Count == 0 || GestureTexts[predictedGesture].gesture.Classe !=
                        app.model.GetPredictedGestures()[t].Classe)
                    {
        ...
        public void DisplayPredictedGesture()
        {
            ...
            foreach (float time in dic.Keys)
            {
                ...
                p.text.text = dic[time].Classe + " at " + string.Format("{0:00}:{1:00}", minutes, seconds);
            }
        }
    }
}